...
module.exports = {
    ...
    staffCombinations: (department_staff_dict, required_staff) => {
      //TODO your code goes here...
        for (let i in department_staff_dict) {
            obj[i].sort((a, b) => a - b);
        }
        let arr = [], b = [];
        for (let i in required_staff) {
            let num = required_staff[i];
            b.push(num);
            for (let j in department_staff_dict) {
                if (i == j) {
                    arr.push(department_staff_dict[j])
                }
            }
        }
      return arr;
    ...
    addStrings: (num1, num2) => {
      //TODO your code goes here...
        ...
        for (let i = num1.length - 1; i >= 0; i--){
            let res = parseInt(num1[i]) + parseInt(num2[i]);
            arr.unshift(res % 10 + a)
            if (res > 10 || res === 10) {
                a = 1;
            } else {
                a = 0;
